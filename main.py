...
class main:
    ...
    def issuebook(self):
        class test(main):
            ...
            def issue(self):
                ...
                self.lb = Label(self.fc, text='REG ID', bg='#ffe8ec', fg='black', font=('times new roman', 11, 'bold'))
            ...
            def issued(self):
                ...
                if self.n<=3:
                    ...
                    book.execute("SELECT * FROM Students WHERE REG='"+self.erpid1+"'")
                    self.issuevar=book.fetchone()
                    self.sum=self.issuevar[11]+1
                    book.execute("UPDATE Students SET NoBook='"+str(self.sum)+"' WHERE REG='"+self.b+"' ")
                    dbstudents.commit()
                
                comm=dbstudents.cursor()
                comm.execute("UPDATE Students SET FromDate='"+str(self.x)+"', ToDate='"+str(self.datecon)+"' , SubmitDate='' WHERE REG='"+self.b+"'")
                dbstudents.commit()
    ...
    def returnbook(self):
        class retu(main):
            def __init__(self):
                ...
                self.label8 = Label(self.f1, text='REG ID', bg='#ffe8ec', fg='black', font=('Times New Roman', 11, 'bold'))
            ...
            def retsucc(self):
                ...
                cursor1.execute("UPDATE Students SET FromDate='',ToDate='',Charge='"+str(self.charge)+"' WHERE REG='"+self.entry+"'")
                dbstudents.commit()
            ...
            def retbook(self):
                ...
                if self.data!=None:
                    if(int(self.data[11])>=1):
                        ...
                        cursor = dbstudents.cursor()
                        cursor.execute("UPDATE Students SET NoBook = 0, SubmitDate='" + str(self.get_date) + "' WHERE REG='" + self.entry + "'")
                        dbstudents.commit()
                    ...
                    messagebox.showerror("Invalid REG ID","This student doesn't exist!")
                    self.entry4.delete(0,END)
    ...
    def code(self):
            ...
            self.btn1=Button(self.fm1,text='Login',fg='black',bg='yellow',width=10,font=('Arial',11,'bold'),activebackground='black',activeforeground='yellow',command=self.login,bd=3,relief='flat',cursor='hand2')
            self.btn1.place(x=25,y=160)
            # self.logo = PhotoImage(file=r'python/avijeets mini project/library-management-files/aa.ico')
            # self.btn1.config(image=self.logo, compound=LEFT)
            # self.small_logo = self.logo.subsample(1, 1)
            # self.btn1.config(image=self.small_logo)


            self.btn2=Button(self.fm1,text='Clear',fg='black',bg='yellow',width=10,font=('Arial',11,'bold'),activebackground='black',activeforeground='yellow',bd=3,relief='flat',cursor='hand2',command=self.mainclear)
            self.btn2.place(x=155,y=160)
            # self.log = PhotoImage(file=r'book.png')
